(function ($) {
    ...
    $.FSRTC = function (options) {
        ...
        if (moz) {
            this.constraints = {offerToReceiveAudio: true, offerToReceiveVideo: false};
        } else {
            this.constraints = {
    ...
    $.FSRTC.prototype.createAnswer = function (sdp) {
        ...
        function onSuccess(stream) {
            ...
            self.peer = RTCPeerConnection({
                ...
                offerSDP: {type: "offer", sdp: self.remoteSDP}
            });
        ...
        if (this.options.videoParams && this.options.videoParams.chromeMediaSource == 'screen') {
            this.options.videoParams = {chromeMediaSource: 'screen', maxWidth: screen.width, maxHeight: screen.height};
            console.error("SCREEN SHARE");
            audio = false;
        } else {
            audio = {mandatory: this.options.audioParams, optional: []};
        }
        ...
        getUserMedia({constraints: {audio: audio}, onsuccess: onSuccess, onerror: onError});
    };
...
(function ($) {
    ...
    $.JsonRpcClient.prototype.socketReady = function () {
        return !(this._ws_socket === null || this._ws_socket.readyState > 1);
...
(function ($) {
    ...
    $.verto.confMan = function (verto, params) {
        /*
         ...
         var html = "<div id='" + div_id + "'><br>" + "<button class='ctlbtn' id='" + play_id + "'>Play</button>" + "<button class='ctlbtn' id='" + stop_id + "'>Stop</button>" + "<button class='ctlbtn' id='" + recording_id + "'>Record</button>" + "<button class='ctlbtn' id='" + rec_stop_id + "'>Record Stop</button>" + "<br><br></div>";
         jq.html(html);
         ...
         var html = "<div id='" + box_id + "'>" + "<button class='ctlbtn' id='" + kick_id + "'>Kick</button>" + "<button class='ctlbtn' id='" + tmute_id + "'>Mute</button>" + "<button class='ctlbtn' id='" + voldn_id + "'>Vol -</button>" + "<button class='ctlbtn' id='" + volup_id + "'>Vol +</button>" + "<button class='ctlbtn' id='" + transfer_id + "'>Transfer</button>" + "</div>";
         jq.html(html);
         if (!jq.data("mouse")) {
         $("#" + box_id).hide();
         }
    ...
    $.verto.confMan.prototype.modCommand = function (cmd, id, value) {
        /*       var confMan = this;
         confMan.verto.sendMethod("verto.broadcast", {"eventChannel": confMan.params.laData.modChannel,"data": {"application": "confSection-control","command": cmd,"id": id,"value": value}});
         */
    };
    ...
    function checkStateChange(oldS, newS) {
        return !!(newS == $.verto.enum.state.purge || $.verto.enum.states[oldS.name][newS.name]);
    ...
    $.verto.dialog.prototype.cidString = function (enc) {
        var dialog = this;
        //phone.incoming(dialog.params.remote_caller_id_name, dialog.params.remote_caller_id_number);
        return dialog.params.remote_caller_id_name + (enc ? " &lt;" : " <") + dialog.params.remote_caller_id_number + (enc ? "&gt;" : ">");
    };
    ...
    $.verto.enum.states = Object.freeze({
        new: {requesting: 1, recovering: 1, ringing: 1, destroy: 1, answering: 1},
        requesting: {trying: 1, hangup: 1},
